...
export default function AccountantWithdrawalsPage() {
  ...
  const canConfirm = useHasPermission("confirmWithdraws");
  const canReject = useHasPermission("rejectWithdraws");
    ...
    { key: "status", header: "Status", cell: (r) => <StatusBadge status={r.status} /> },
    {
      ...
      cell: (r) => (
        <div className="flex items-center gap-1.5">
          <Button size="icon-sm" variant="ghost" onClick={() => setViewTarget(r)}>
        ...
        <Modal open={!!viewTarget} onClose={() => setViewTarget(null)} title="Withdrawal Details">
          <div className="space-y-4 text-sm">
            <div className="space-y-3">
              {([
                ["Client", viewTarget.fullName],
                ["Bank", viewTarget.bankToTransferTo],
                ["Account", viewTarget.accountNumber],
                ["Recipient", viewTarget.recipientName],
