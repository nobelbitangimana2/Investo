...
import { getWithdrawals, confirmWithdrawalApi, rejectWithdrawalApi, getAccountantPermissions } from '@/lib/api';
...
import type { Withdrawal, AccountantPermissions } from '@/types';

export default function AccountantWithdrawalsScreen() {
  ...
  async function load() {
    if (!user) return;
    const [w, p] = await Promise.all([getWithdrawals(false), getAccountantPermissions(user.id)]);
    setWithdrawals(w); setPerms(p);
  }
  ...
  async function handleAction(note?: string) {
    ...
    if (action.type === 'confirm') {
      ...
      Alert.alert(t('common.success'), t('accountant.withdrawals.withdrawalConfirmed'));
    } else {
      const updated = await rejectWithdrawalApi(action.w.id, note ?? '');
      setWithdrawals((prev) => prev.map((w) => w.id === updated.id ? updated : w));
      Alert.alert(t('common.success'), t('accountant.withdrawals.withdrawalRejected'));
    }
  }

  const canConfirm = user?.role === 'admin' || perms?.confirmWithdraws;
  const canReject = user?.role === 'admin' || perms?.rejectWithdraws;

  return (
    <View style={styles.root}>
      <Header />
      <Text style={styles.pageTitle}>{t('accountant.withdrawals.title')}</Text>
      <FlatList
        data={withdrawals}
        keyExtractor={(w) => w.id}
